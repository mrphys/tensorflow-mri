...
class LSTMConvBlock(tf.keras.layers.Layer):
  """An LSTM convolution block.
  ...
  def __init__(self,
    ...
    conv = layer_util.get_nd_layer('ConvLSTM', self._rank)
    ...
    if self._use_dropout:
      if self._dropout_type == 'standard':
        #dropout = tf.keras.layers.Dropout
        dropout  = lambda *args, **kwargs: tf.keras.layers.TimeDistributed(
                    tf.keras.layers.Dropout(*args, **kwargs))

      elif self._dropout_type == 'spatial':
        #dropout = layer_util.get_nd_layer('SpatialDropout', self._rank)
        dropout = lambda *args, **kwargs: tf.keras.layers.TimeDistributed(
          layer_util.get_nd_layer('SpatialDropout', self._rank)(*args, **kwargs))
    if tf.keras.backend.image_data_format() == 'channels_last':
      self._channel_axis = -1
    else:
      self._channel_axis = 1
    ...
    #Kernel_Regularizer (allow input of function)
    if self._kernel_regularizer is not None and (self._kernel_regularizer.startswith('tf.') | self._kernel_regularizer.startswith('lambda')):
      _kernel_regularizer = eval(self._kernel_regularizer)
    else:
      _kernel_regularizer = self._kernel_regularizer
    #Recurrent_Regularizer (allow input of function)
    if self._recurrent_regularizer is not None and (self._recurrent_regularizer.startswith('tf.') | self._recurrent_regularizer.startswith('lambda')):
      _recurrent_regularizer = eval(self._recurrent_regularizer)
    else:
      _recurrent_regularizer = self._recurrent_regularizer
