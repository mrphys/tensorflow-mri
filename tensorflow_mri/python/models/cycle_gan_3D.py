...
"""CycleGAN 3D Layers and Composite Generator/Discriminator Model Implementation."""
...
class CG3DResNetBlock(Layer):
	...
	def __init__(self, n_filters):
		...
		"""
		super(CG3DResNetBlock, self).__init__()
		self.init = RandomNormal(stddev=0.02)
		self.conv2a = Conv3D(n_filters, (3,3,3), padding="same", kernel_initializer=self.init)
		self.conv2b = Conv3D(256, (3,3,3), padding="same", kernel_initializer=self.init)
...
class CG3DEncoder(Layer):
	"""Encoder Layer specific to the CycleGAN 3D Generator Architecture."""

	def __init__(self):
		super(CG3DEncoder, self).__init__()
		self.init = RandomNormal(stddev=0.02)
		self.conv2a = Conv3D(64, (7,7,7), padding='same', kernel_initializer=self.init)
		self.conv2b = Conv3D(128, (3,3,3), strides=(2,2,2), padding='same', kernel_initializer=self.init)
		self.conv2c = Conv3D(256, (3,3,3), strides=(2,2,2), padding='same', kernel_initializer=self.init)
		self.instance_norm1 = InstanceNormalization(axis=-1)
		self.instance_norm2 = InstanceNormalization(axis=-1)
...
class CG3DDecoder(Layer):
	"""Decoder Layer specific to the CycleGAN 3D Generator Architecture."""
	def __init__(self):
		super(CG3DDecoder, self).__init__()
		self.init = RandomNormal(stddev=0.02)
		self.conv2ta = Conv3DTranspose(128, (3,3,3), strides=(2,2,2), padding='same', kernel_initializer=self.init)
		self.conv2tb = Conv3DTranspose(64, (3,3,3), strides=(2,2,2), padding='same', kernel_initializer=self.init)
		self.conv2c = Conv3D(1, (7,7,7), padding='same', kernel_initializer=self.init)
		self.instance_norm1 = InstanceNormalization(axis=-1)
		self.instance_norm2 = InstanceNormalization(axis=-1)
		self.instance_norm3 = InstanceNormalization(axis=-1)
...
class CG3DConvEncoder(Layer):
	"""Convolutional Encoder Layer specific to the CycleGAN 3D Discriminator Architecture."""
	def __init__(self):
		super(CG3DConvEncoder, self).__init__()
		self.init = RandomNormal(stddev=0.02)

		self.conv2a = Conv3D(64, (4,4,4), strides=(2,2,2), padding='same', kernel_initializer=self.init)
		self.conv2b = Conv3D(128, (4,4,4), strides=(2,2,2), padding='same', kernel_initializer=self.init)
		self.conv2c = Conv3D(128, (4,4,4), padding='same', kernel_initializer=self.init)
		self.conv2d = Conv3D(1, (4,4,4), padding='same', kernel_initializer=self.init)
	...
	def call(self, input_tensor):
		...
		out_pred = self.conv2d(x)

		return out_pred

class CG3DGenerator(Model):
	"""CycleGAN 3D Generator Model."""
	def __init__(self, image_shape, n_resnet=None):
		super(CG3DGenerator, self).__init__()
		self.n_resnet = n_resnet

		# If image size is less than 256x256, or there is no arg - only use
		# 6 resnet blocks
		if (image_shape[1] < 256 and n_resnet is None) or n_resnet is None:
			self.n_resnet = 6

		self.encoder = CG3DEncoder()
		self.decoder = CG3DDecoder()
		self.res_blocks = []

	def call(self, inputs):
		x = self.encoder(inputs)
		for i in range(0, self.n_resnet):
			self.res_blocks.append(CG3DResNetBlock(256))
			x = self.res_blocks[i](x)
		x = self.decoder(x)
		return x

class CG3DDiscriminator(Model):
	"""CycleGAN Discriminator Model."""
	def __init__(self):
		super(CG3DDiscriminator, self).__init__()
		self.conv_encoder = CG3DConvEncoder()
...
class CycleGAN3D(Model):
	...
	def __init__(
		self,
		generator_G = CG3DGenerator(image_shape=(20,1,128,128)),
		generator_F = CG3DGenerator(image_shape=(20,1,128,128)),
		discriminator_X = CG3DDiscriminator(),
		discriminator_Y = CG3DDiscriminator(),
		lambda_cycle=10.0,
		lambda_identity=1.0,
	):
		super(CycleGAN3D, self).__init__()
	...
	def compile(
		...
		# Loss function for the Cycle Loss and Identity Loss (can be customised)
		cycle_loss_fn=MeanAbsoluteError(),
		identity_loss_fn=MeanAbsoluteError(),
		**kwargs
	):

		super(CycleGAN3D, self).compile()
	...
	def predict_step(self, data):
		raise NotImplementedError("You can't call `model.predict()` on the \n"
		"CycleGAN Model! You must call this on the Generator sub-model with \n"
		"model.gen_G() or model.gen_F().")
