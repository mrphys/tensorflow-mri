...
UNET_DOC_TEMPLATE = string.Template(
  ...
  Args:
    ...
    stateful: False -> to reset between each batch set to False at inference set to True.
    return_sequences: True -> to treat sequence per sequence at training set to true.
...
class LSTMUNet(tf.keras.Model):
  """LSTM U-Net model (private base class)."""
  def __init__(self,
               rank,
               filters,
               kernel_size,
    ...
    block_layer = model_util.get_nd_model('LSTMConvBlock', self._rank)
    block_config = dict(
  ...
  def reset_states(self):
    """Reset different layers states"""
    for layer in self._enc_blocks:
      layer.reset_states()
    for layer in self._dec_blocks:
      layer.reset_states()
    self._out_block.reset_states()
...
@api_util.export("models.LSTMUNet2D")
@tf.keras.utils.register_keras_serializable(package='MRI')
class LSTMUNet2D(LSTMUNet):
