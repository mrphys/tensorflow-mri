...
class ReconstructTest(test_util.TestCase):
  ...
  def test_cs(self):
    """Test compressed sensing reconstruction."""
    # image = image_ops.phantom(dtype=tf.complex64)
    # numel = image.shape.num_elements()

    # sampling_density = 0.25
    # mask = self._random_mask(image.shape, 0.25, calib_shape=[24, 24])

    # kspace = fft_ops.fftn(image, shift=True)
    # kspace *= tf.cast(mask, kspace.dtype)
    # image = fft_ops.ifftn(kspace, shift=True)

    import scipy.io as sio
    data = sio.loadmat('tests/data/cs_shepp_logan_tv.mat')
    image = data['image']
    mask = data['mask']
    pdf = data['pdf']
    ...
    kspace = fft_ops.fftn(image, shift=True)
    kspace *= tf.cast(mask, kspace.dtype)
    image = fft_ops.ifftn(kspace / pdf, shift=True)

    result = recon_ops.reconstruct(kspace, mask=mask, method='cs',
                                   initial_x=image)
